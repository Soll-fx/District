...
export interface ProgressMetricCardProps {
  ...
  accent?: MetricAccent;
  /** Одна серия. Либо `series`. */
  data?: SeriesPoint[];
  /** Несколько именованных серий. Приоритетнее `data`. */
  series?: MetricSeries[];
  size?: CardSize;
  showStats?: boolean;
...
const SIZES: Record<CardSize, { minH: string; pad: string; footer: string; title: string; headline: string }> = {
  ...
  md: { minH: "min-h-[380px]", pad: "px-8 pt-7", footer: "px-8 py-4", title: "text-[17px]", headline: "text-[40px]" },
...
export function ProgressMetricCard({
  ...
  const sz = SIZES[size];
  const shell = `relative flex ${sz.minH} w-full flex-col overflow-hidden rounded-[28px] border border-card-border bg-card shadow-[0_2px_10px_rgba(0,0,0,0.04)] ${className}`;
      ...
      <div className={shell} aria-busy="true">
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="h-5 w-24 animate-pulse rounded bg-bg" />
          </div>
          <div className="mt-6 h-14 w-48 animate-pulse rounded-lg bg-bg" />
          <div className="mt-auto h-24 w-full animate-pulse rounded-lg bg-bg/50" />
        </div>
        <div className={`border-t border-text-1/[0.06] ${sz.footer}`}>
  ...
  if (!hasData) {
    return (
      <div className={shell}>
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          <h3 className={`${sz.title} font-semibold tracking-tight text-text-1`}>{title}</h3>
          <div className="flex flex-1 flex-col items-center justify-center gap-1 py-10 text-center">
            <p className="text-sm font-medium text-text-1">No data yet</p>
            <p className="text-xs text-text-3">Metrics will appear once data is available.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={shell}>
      {/* Зона графика (справа, за контентом) */}
      <div className="absolute inset-y-0 right-0 z-0" style={{ width: `${REGION_W}%` }}>
        ...
        />
        <div
          className="pointer-events-none absolute inset-0 text-text-1/[0.13]"
          style={{
      ...
      <div className={`pointer-events-none relative z-10 flex flex-1 flex-col ${sz.pad}`}>
        ...
        {isMulti && (
          <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1">
            {chartSeries.map((s) => (
              <span key={s.name} className="flex items-center gap-1.5 text-[12px] text-text-3">
        ...
        <div className={`mt-5 ${sz.headline} font-medium leading-none tracking-tight text-text-1`}>{displayTotal}</div>
      </div>

      {/* Непрозрачный футер: delta слева, вторичные статы справа */}
      <div
        className={`relative z-10 flex items-center justify-between gap-4 border-t border-text-1/[0.06] bg-card ${sz.footer} text-[14px]`}
      >
        <div>
        ...
        {showStats && (
          <div className="flex items-center gap-2.5 text-[12px] text-text-3">
            <span>
              <span className="font-medium text-text-1/80">{fmtCompact(stats.peak)}</span> peak
            </span>
            <span className="opacity-40">·</span>
            <span>
              <span className="font-medium text-text-1/80">{fmtCompact(stats.low)}</span> low
            </span>
            <span className="opacity-40">·</span>
            <span>
              <span className="font-medium text-text-1/80">{fmtCompact(Math.round(stats.avg))}</span> avg
            </span>
          </div>
        )}
      </div>
